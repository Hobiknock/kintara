...
async function tgCall(method, body, tries = 3) {
  if (!TG_API) return null;
  for (let t = 1; t <= tries; t++) {
    const ac = new AbortController();
    const to = setTimeout(() => ac.abort(), 25000); // hard 25 dtk — fetch GAK BOLEH nge-hang selamanya (bot jadi tuli)
    try {
      ...
      clearTimeout(to);
      // self-heal: blip jaringan (fetch failed) → retry backoff, jangan nyerah — dulu 1x gagal = bot tuli jam-jaman
      if (t < tries) { await new Promise((r) => setTimeout(r, 1000 * t * t)); continue; }
...
async function send(text, opts = {}) {
  ...
  // Fallback: kalau HTML entity rusak (mis. error server berisi <!doctype>),
  // kirim ulang sebagai plain text agar command tidak silent-fail.
  const fallback = {
    ...body,
...
async function pollCommands(commands, opts = {}) {
  ...
  if (!res?.ok) return false; // sinyal gagal → ctl hitung & exit kalau beruntun
  ...
  return true; // poll sehat — reset counter fetch-failure di ctl
}

async function setMyCommands(cmds) {
  if (!TG_API) return;
  try {
    const res = await tgCall('setMyCommands', { commands: cmds });
    if (!res?.ok) console.error('[telegram] setMyCommands gagal:', res?.description);
  } catch (e) {
    console.error('[telegram] setMyCommands err:', e.message);
  }
}

module.exports = { send, editMessage, answerCallback, pollCommands, setMyCommands };
