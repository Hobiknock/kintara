...
class KintaraClient {
  ...
  static async create(opts = {}) {
    ...
    if (client.cookie && !opts.forceLogin) { // NOTE: cookie auth short-circuit skipped saat opts.privateKey di-set (setkey)
      try {
        const me = await client.me();
        client.player = me?.player || client.player;
        return { client, player: client.player };
      } catch (e) {
        if (e?.code === 'MAINTENANCE') throw e;
      }
    }
   ...
   */
  async _request(method, base, pathname, body = null, timeoutMs = DEFAULT_TIMEOUT, _retried = false, _502attempt = 0) {
    if (!this.cookie) {
      if (this.autoLogin) {
    ...
    // Proxy opsional (mis. WARP SOCKS5) — diaktifkan via env KINTARA_PROXY=socks5://127.0.0.1:40000
    const dispatcher = require('../lib/proxyDispatcher')();
    ...
    try {
      ...
      if (!json) {
        if (isMaintenanceResponse(res.status, text)) {
          const err = new Error(`Kintara API is under maintenance right now (status ${res.status}). Please try again later.`);
          err.code = 'MAINTENANCE';
          err.status = res.status;
          throw err;
        }
        // 502 Cloudflare gateway — retry with backoff
        if (res.status === 502 && _502attempt < 2) {
          clearTimeout(timer);
          const wait = (5000 + Math.floor(Math.random() * 3000)) * (_502attempt + 1);
          await new Promise((r) => setTimeout(r, wait));
          return this._request(method, base, pathname, body, timeoutMs, _retried, _502attempt + 1);
        }
        // Non-JSON response (kemungkinan HTML login page -> cookie expired)
        if (this.autoLogin && !_retried) {
          clearTimeout(timer);
          await this.ensureLogin(true);
          return this._request(method, base, pathname, body, timeoutMs, true, _502attempt);
        }
        throw new Error(`Non-JSON response (status ${res.status}) — kemungkinan cookie expired/invalid. Body: ${text.slice(0, 200)}`);
      }

      if (!res.ok) {
        // 502 gateway — retry up to 2x with backoff (Cloudflare intermittent)
        if (res.status === 502 && _502attempt < 2) {
          clearTimeout(timer);
          const wait = (5000 + Math.floor(Math.random() * 3000)) * (_502attempt + 1);
          await new Promise((r) => setTimeout(r, wait));
  ...
  /** /api/servers (infra baru per-zone). */
  servers() { return this.get('/api/servers'); }

  /** Auto-pick server joinable: full=false, requiresMembership=false, queueLength terkecil.
   *  @returns {Promise<{id,shardId,wsBaseUrl,baseUrl,fanoutOrigin,server:object}>} shardId='s<routeShardId>' */
  async resolveServer(pick = {}) {
    const now = Date.now();
    if (this._resolveCache && now - this._resolveCache.ts < 60000) return this._resolveCache.value;
    const r = await this.servers();
    const list = (r && Array.isArray(r.servers)) ? r.servers : [];
    if (!list.length) throw new Error('resolveServer: /api/servers kosong');
    const minLevel = Number(pick.minLevel != null ? pick.minLevel : 1);
    const zone = (pick.zone || process.env.KINTARA_ZONE || '').toLowerCase();
    const joinable = list.filter((s) => s && s.id != null && !s.full && !s.requiresMembership && Number(s.minLevel || 0) <= minLevel);
    let pool = joinable.length ? joinable : list;
    if (zone) {
      const inZone = pool.filter((s) => String(s.zone || '').toLowerCase() === zone);
      if (inZone.length) pool = inZone; // kalau zona pilihan kosong, fallback ke semua
    }
    pool.sort((a, b) => (Number(a.queueLength) || 0) - (Number(b.queueLength) || 0));
    // KINTARA_FORCE_SERVER=<id>: paksa server tertentu (mis. 15) — override auto-pick
    const forced = String(process.env.KINTARA_FORCE_SERVER || '');
    const fsrv = forced ? pool.find((s) => String(s.id) === forced) : null;
    const s = fsrv || pool[0];
    const val = { id: s.id, shardId: 's' + s.routeShardId, wsBaseUrl: s.wsBaseUrl, baseUrl: s.baseUrl, fanoutOrigin: s.fanoutOrigin, server: s };
    this._resolveCache = { ts: now, value: val };
    return val;
  }
  ...
  fishingSuppliesBuy(itemType, counter = 'pond') { return this.post('/api/auth/fishing-supplies-buy', { itemType, counter }); }
  fishingExchange(exchange) { return this.post('/api/auth/fishing-exchange', { exchange }); }
